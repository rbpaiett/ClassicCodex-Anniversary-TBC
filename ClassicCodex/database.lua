...
-- for base data
if not devMode then
    CodexDB.missingReport = {}
    local function reportMissing(name, key, notice)
    ...
    setmetatable(CodexDB['quests']["data"], {
        __index = function(self, key)
            reportMissing('quests', key, L['[ClassicCodex] The quest #%s is missing.'])
            return {}
        end
    })
    setmetatable(CodexDB['items']["data"], {
        __index = function(self, key)
            reportMissing('items', key, L['[ClassicCodex] The item #%s is missing.'])
            return {}
        end
    })
    setmetatable(CodexDB['objects']["data"], {
        __index = function(self, key)
            reportMissing('objects', key, L['[ClassicCodex] The object #%s is missing.'])
            return {}
        end
    })
    setmetatable(CodexDB['units']["data"], {
        __index = function(self, key)
            reportMissing('units', key, L['[ClassicCodex] The unit #%s is missing.'])
            return {}
        end
    })
    -- for locales
    setmetatable(CodexDB['quests']["loc"], {
        __index = function(self, key)
            local notice = reportLocalMissing('quests', key, L["The {locale} locale text of the quest #{id} is missing."])
            return {
                ["D"] = notice,
                ["O"] = notice,
                ["T"] = notice
    ...
    setmetatable(CodexDB['items']["loc"], {
        __index = function(self, key)
            return reportLocalMissing('items', key, L["The {locale} locale text of the item #{id} is missing."])
        end
    })
    setmetatable(CodexDB['objects']["loc"], {
        __index = function(self, key)
            return reportLocalMissing('objects', key, L["The {locale} locale text of the object #{id} is missing."])
        end
    })
    setmetatable(CodexDB['professions']["loc"], {
        __index = function(self, key)
            return reportLocalMissing('professions', key, L["The {locale} locale text of the profession #{id} is missing."])
        end
    })
    setmetatable(CodexDB['units']["loc"], {
        __index = function(self, key)
            return reportLocalMissing('units', key, L["The {locale} locale text of the unit #{id} is missing."])
        end
    })
    setmetatable(CodexDB['zones']["loc"], {
        __index = function(self, key)
            return reportLocalMissing('zones', key, L["The {locale} locale text of the zone #{id} is missing."])
        end
    })
end
...
function CodexDatabase:SearchItemById(id, meta, maps, allowedTypes)
    -- ADD THIS LINE BELOW:
    if not self.LootCodexImported then self:ImportLootCodex(); self.LootCodexImported = true end
...
function CodexDatabase:ImportLootCodex()
    if self.LootCodexImported or not LootCodexDB then return end
    
    local ccItems = CodexDB["items"]["data"]
    if not ccItems then return end

    -- 1. Create a "Safe Zone" for our data so we don't touch protected tables
    local function SecureInject(itemID, ownerID, dropRate, typeKey)
        if not itemID or not ccItems[itemID] then return end
        
        -- If the item is just a number (uninitialized), make it a table
        if type(ccItems[itemID]) ~= "table" then
            ccItems[itemID] = {} 
        end
        
        -- Create the folder only if we actually can
        if not ccItems[itemID][typeKey] then
            -- Use a protected call (pcall) to prevent the "nil index" crash
            pcall(function() ccItems[itemID][typeKey] = {} end)
        end
        
        -- Final safety check: if it's still nil, the table is locked; skip it.
        if ccItems[itemID][typeKey] then
            ccItems[itemID][typeKey][ownerID] = dropRate
        end
    end

    -- 2. PROCESS NPC LOOT
    for npcID, lootTable in pairs(LootCodexDB) do
        if type(npcID) == "number" and type(lootTable) == "table" then
            for _, entry in ipairs(lootTable) do
                if type(entry) == "table" then
                    local itemID = entry[1]
                    local rate = entry[3] or 0
                    
                    -- Mode 174/14 check
                    if entry[5] and type(entry[5]) == "table" then
                        rate = entry[5][174] or entry[5][14] or rate
                    end
                    
                    SecureInject(itemID, npcID, rate, "U")
                end
            end
            
            -- References (World Drops)
            if lootTable.r and LootCodexDB_Refs then
                for _, refEntry in ipairs(lootTable.r) do
                    local refData = LootCodexDB_Refs[refEntry[1]]
                    local scale = (refEntry[2] or 100) / 100
                    if type(refData) == "table" then
                        for _, itemEntry in ipairs(refData) do
                            local itemID = itemEntry[1]
                            local rate = (itemEntry[3] or 0) * scale
                            SecureInject(itemID, npcID, rate, "U")
                        end
                    end
                end
    ...
    -- 3. PROCESS OBJECTS
    if LootCodexDB_Objects and LootCodexDB_ObjectIDs then
        local nameToId = {}
        for id, name in pairs(LootCodexDB_ObjectIDs) do nameToId[name] = id end

        for objName, lootTable in pairs(LootCodexDB_Objects) do
            local objID = nameToId[objName]
            if objID and type(lootTable) == "table" then
                for _, entry in ipairs(lootTable) do
                    if type(entry) == "table" then
                        SecureInject(entry[1], objID, entry[3] or 0, "O")
                    end
                end
            end
        end
    ...
    self.LootCodexImported = true
    print("|cff00ff00ClassicCodex: Loot Data Injected via Protected Mode.|r")
end