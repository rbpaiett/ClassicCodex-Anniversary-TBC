...
function CodexQuest:UpdateQuestLog()
    ...
    -- iterate over all quests (40 is the safe upper limit for TBC)
    for questLogId = 1, 40 do
        local _, _, _, header, _, complete, _, questId = GetQuestLogTitle(questLogId)
        
        if not header and questId and quests[questId] then
            ...
            if objectives then
                ...
                for i=1, objectives do
                    ...
                    if text then
                        ...
                        if obj then
                            local nNum = tonumber(objNum) or 0
                            local nNeed = tonumber(objNeeded) or 0
                            state = state .. i .. (((nNum >= nNeed) or done) and "done" or "todo")
                        end
                    end
                end
    ...
    -- Restore header states (close what we opened)
    for _, index in ipairs(collapsedHeaders) do
    ...
    if CodexConfig.trackingMethod ~= 4 and table.getn(CodexQuest.queue) > 0 then
        for _, entry in pairs(CodexQuest.queue) do
            if CodexConfig.trackingMethod ~= 2 or entry[3] then
                CodexMap:DeleteNode("CODEX", entry[1])
                if CodexConfig.trackingMethod ~= 3 then
                    local meta = {["addon"] = "CODEX", ["questLogId"] = entry[3]}
                    for _, qId in pairs(entry[2]) do
                        CodexDatabase:SearchQuestById(qId, meta)
                    end
                end
                CodexQuest.updateNodes = true
            end
...
function CodexQuest:AddWorldMapIntegration()
    ...
    CodexQuest.mapButton:SetPoint("TOPRIGHT", 2, -60)
    WorldMapFrameCloseButton:SetFrameLevel(CodexQuest.mapButton:GetFrameLevel() + 1)
